...
1. **Сначала ИЗМЕРЯЕМ** (базовые метрики)
2. **Интерпретируем отчёт** (hot path, тяжёлые аллокации, блокировки)
3. **Вносим МИНИМАЛЬНУЮ оптимизацию**, снова измеряем
4. **Фиксируем результат в отчёте** (до/после)
